...
from .ops.modules import MSDeformAttn_3D
from .position_encoding_3D import build_position_encoding_3D

class DeformableTransformer3D(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        for m in self.modules():
            if isinstance(m, MSDeformAttn_3D):
                m._reset_parameters()
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        # self attention
        self.self_attn = MSDeformAttn_3D(d_model, n_levels, n_heads, n_points)
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def get_reference_points(spatial_shapes, valid_ratios, device):
        ...
        for lvl, (D_, H_, W_) in enumerate(spatial_shapes):
            ...
            valid_ratios  = valid_ratios.cuda()
